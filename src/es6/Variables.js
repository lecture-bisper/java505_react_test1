...
// 변수의 스코프(범위) 함수를 기준으로 하고 있음, for 및 if, switch ~ case 내에서 선언한 변수라도 해당 코드블럭을 벗어나서 사용이 가능함
// 중복 선언이 가능함

// let : ES6에서 추가된 변수 선언 방식
// 변수의 스코프가 코드 블럭( { } )을 기준으로 함, for 및 if, switch ~ case 의 코드블럭 내에서 선언된 변수는 해당 코드블럭을 벗어나면 메모리에서 삭제됨
// 중복 선언이 불가능함

// const : ES6에서 추가된 상수 선언 방식
// 변수의 스코프가 코드 블럭( { } )을 기준으로 함, for 및 if, switch ~ case 의 코드블럭 내에서 선언된 변수는 해당 코드 블럭을 벗어나면 메모리에서 삭제됨
// 중복 선언이 불가능함
